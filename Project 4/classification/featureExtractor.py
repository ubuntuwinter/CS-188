...
class KMeansClusterDigit(BaseFeatureExtractor):
    ...
    def fit(self, trainingData):
        ...
        # new_cluster_no = copy.deepcopy(cluster_no)

        # performing K-means clustering
        # YOU SHOULD use the cluster_no for computing the clusters
        "*** YOUR CODE HERE ***"
        for iteration in range(self.num_iterations):
            clusters = [[] for k in range(self.num_cluster)]
            for i, j in enumerate(cluster_no):
                clusters[j].append(i)
            for i in range(self.num_cluster):
                self.clusters[i] = np.mean(trainingData[clusters[i]], axis=0)
            u2 = np.sum(self.clusters * self.clusters, axis=1)
            x_iu_j = np.dot(trainingData, self.clusters.T)
            new_cluster_no = np.argmin(-2 * x_iu_j + u2, axis=1)
            '''
            for i in range(n):
                xi = np.expand_dims(trainingData[i], 0).repeat(self.num_cluster, axis=0)
                ui = self.clusters
                new_cluster_no[i] = np.argmin(np.linalg.norm(xi - ui, axis=1))
            '''
            if np.sum(new_cluster_no == cluster_no) == n:
                break
            # cluster_no = copy.deepcopy(new_cluster_no)
...
if __name__ == '__main__':
    ...
    parser.add_option('-f', '--feature', help=default('Unsupervised method'), choices=['basic', 'pca', 'kmeans'],
                      default='kmeans')
    parser.add_option('-s', '--size', help=default('Dimension size (PCA) or cluster size (KMeans)'), default=10,
                      type=int)
