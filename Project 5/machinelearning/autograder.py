...
def verify_node(node, expected_type, expected_shape, method_name):
    ...
    if expected_type != 'loss':
        assert all([(expected is '?' or actual == expected) for (actual, expected) in
                    zip(node.data.size(), expected_shape)]), (
...
def check_digit_classification(tracker):
    ...
    if test_accuracy >= accuracy_threshold:
        ...
        print(
            "Your final test set accuracy ({:.2%}) must be at least {:.0%} to receive full points for this question".format(
...
def check_adversarial_examples(tracker):
    ...
    if test_accuracy <= accuracy_threshold:
        ...
        print(
            "Your final test set accuracy ({:.2%}) must be at most {:.0%} to receive full points for this question".format(
                test_accuracy, accuracy_threshold))
